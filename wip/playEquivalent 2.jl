...
# Examples to check the functioning of the DC flow, drawing, with or without equialents.
eq_sub = equivalent_parameters(sub2, eq_buses, outages=[("G", "I")], true)
rem = copy(remaining2)
outages_rem = [("B", "E"), ("F", "K"), ("I", "N"), ("C", "D")]
calcanddraw(rem, outages=outages_rem, eq=eq_sub, layout=coord)

eq_rem = equivalent_parameters(remaining2, eq_buses, outages=outages_rem, true)
sub = copy(sub2)
outages_sub = [("G", "I")]
calcanddraw(sub, outages=outages_sub, eq=eq_rem, layout=coord)

calcanddraw(g, outages=[outages_rem; ("G", "I")], layout=coord)
...
################################
eq_sub3 = equivalent_parameters(sub2, eq_buses, true)
...
eq_outage3 = equivalent_parameters(remaining2, eq_buses, outages=res_remaining3, true)
